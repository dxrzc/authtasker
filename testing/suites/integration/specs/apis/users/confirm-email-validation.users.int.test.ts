...
describe('POST /api/users/confirmEmailValidation/:token', () => {
    describe('Token', () => {
        test('return status 400 BAD REQUEST if token purpose is not valid', async () => {
            ...
            const response = await request(testKit.server).get(
                `${testKit.endpoints.confirmEmailValidation}/${emailValidationToken}`,
            );
    ...
    describe('Database Operations', () => {
        ...
        test('throw status 404 NOT FOUND if user in token does not exist', async () => {
            ...
            // Confirm email validation
            const response = await request(testKit.server).get(
                `${testKit.endpoints.confirmEmailValidation}/${tokenInEmail}`,
            );
        ...
        test('blacklist token provided in url', async () => {
            ...
            const blacklistedToken = await testKit.jwtBlacklistService.tokenInBlacklist(
